...
vpn_peer_exports_test() ->
    ...
    ?assert(erlang:function_exported(vpn_peer, debug_frame_history, 1)),
...
vpn_ovpn_envelope_exports_test() ->
...
vpn_manager_exports_test() ->
    ...
    ?assert(erlang:function_exported(vpn_manager, debug_frame_history, 1)),
...
vpn_http_exports_test() ->
    ?assertMatch({module, vpn_http}, code:ensure_loaded(vpn_http)),
    ?assert(erlang:function_exported(vpn_http, start_link, 0)),
    ?assert(erlang:function_exported(vpn_http, stop, 0)),
    ...
    ?assertMatch({module, vpn_n2o_admin}, code:ensure_loaded(vpn_n2o_admin)),
    ?assert(erlang:function_exported(vpn_n2o_admin, init, 2)),
    ?assert(erlang:function_exported(vpn_n2o_admin, cx, 2)),
    ?assert(erlang:function_exported(vpn_n2o_admin, event, 1)),
    ?assert(erlang:function_exported(vpn_n2o_admin, refresh_dashboard, 0)),
...
vpn_link_exports_test() ->
    ...
    ?assert(erlang:function_exported(vpn_link, debug_frame_history, 1)),
